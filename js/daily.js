...
function initDailyPage(){
  ...
  const openWeekWidgetBtn=document.getElementById('openDailyWeekWidgetBtn');
  openWeekWidgetBtn?.addEventListener('click',()=>{
    const periodWrap=document.getElementById('dailyPeriodNavWrap');
    if(periodWrap) periodWrap.style.display='none';
    widgetDailyWeek?.();
  });

  const openMonthWidgetBtn=document.getElementById('openDailyMonthWidgetBtn');
  openMonthWidgetBtn?.addEventListener('click',()=>{
    const periodWrap=document.getElementById('dailyPeriodNavWrap');
    if(periodWrap) periodWrap.style.display='none';
    widgetDailyMonth?.();
  });
...
function widgetDailyWeek(){
  return makeWidget('Daily - Week',(isPopup,win)=>{
    const doc=win.document;
    const W=doc.createElement('div');
    W.style.cssText='display:flex;flex-direction:column;height:100%;box-sizing:border-box;padding:8px 12px;';
    ...
    grid.style.cssText='display:grid;grid-template-columns:repeat(7,1fr);gap:6px;flex:1;min-height:0;overflow:auto;';
    W.append(grid);
    ...
    const render=()=>{
      grid.innerHTML='';
      const today=new Date(dailySelectedDate||new Date());
      today.setHours(0,0,0,0);
      const dayOfWeek=today.getDay();
      const startOfWeek=new Date(today);
      startOfWeek.setDate(today.getDate()-dayOfWeek);
      ...
      for(let i=0;i<7;i++){
        ...
        const dstr=y+'-'+m+'-'+d;
        const list=JSON.parse(win.localStorage.getItem('memo2.daily.'+dstr)||'[]');
        ...
        col.style.cssText='display:flex;flex-direction:column;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;';

        const header=doc.createElement('div');
        header.style.cssText=`padding:4px 6px;text-align:center;font-size:11px;font-weight:600;background:${isToday?'#5C8DFF':'#f8f9fa'};color:${isToday?'#fff':'#374151'};`;
        header.textContent=weekdays[i]+' '+date.getDate();

        const body=doc.createElement('div');
        body.style.cssText='padding:4px 6px;flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:3px;';

        if(!list.length){
          const empty=doc.createElement('div');
          empty.textContent='—';
          empty.style.cssText='color:#d1d5db;font-size:11px;text-align:center;padding:4px 0;';
          body.append(empty);
        }else{
          list.forEach(item=>{
            const row=doc.createElement('div');
            ...
            cb.type='checkbox';
            cb.checked=!!item.done;
            cb.style.cssText='width:12px;height:12px;cursor:pointer;flex-shrink:0;margin-top:2px;accent-color:#5C8DFF;';

            const txt=doc.createElement('span');
            txt.textContent=item.text;
            txt.style.cssText=item.done
              ?'font-size:11px;text-decoration:line-through;color:#9aa5b1;word-break:break-all;'
              :'font-size:11px;color:#374151;word-break:break-all;';

            cb.addEventListener('change',()=>{
              const list2=JSON.parse(win.localStorage.getItem('memo2.daily.'+dstr)||'[]');
              const target=list2.find(t=>t.text===item.text);
              if(target){
                target.done=cb.checked;
                win.localStorage.setItem('memo2.daily.'+dstr,JSON.stringify(list2));
                txt.style.textDecoration=cb.checked?'line-through':'';
                txt.style.color=cb.checked?'#9aa5b1':'#374151';
              }
            });
...
function widgetDailyMonth(){
  return makeWidget('Daily - Month',(isPopup,win)=>{
    const doc=win.document;
    const W=doc.createElement('div');
    W.style.cssText='display:flex;flex-direction:column;height:100%;box-sizing:border-box;padding:8px 12px;';

    const title=doc.createElement('div');
    title.textContent='🗓️ Monthly Tasks';
    title.style.cssText='font-weight:700;font-size:14px;margin-bottom:8px;';
    W.append(title);

    const grid=doc.createElement('div');
    grid.style.cssText='display:grid;grid-template-columns:repeat(7,1fr);gap:4px;flex:1;min-height:0;overflow:auto;';
    W.append(grid);
    ...
    const render=()=>{
      ...
      const base=new Date(dailySelectedDate||new Date());
      base.setHours(0,0,0,0);
      const y=base.getFullYear();
      const m=base.getMonth();

      const weekdayHeaderRow=doc.createElement('div');
      weekdayHeaderRow.style.cssText='display:contents;';
      weekdays.forEach(name=>{
        const cell=doc.createElement('div');
        cell.textContent=name;
        cell.style.cssText='text-align:center;font-size:10px;font-weight:600;color:#6b7280;padding:2px 0;';
        grid.append(cell);
      });
      ...
      for(let i=0;i<firstDay;i++){
        grid.append(doc.createElement('div'));
      }
      ...
      for(let d=1;d<=totalDays;d++){
        ...
        const list=JSON.parse(win.localStorage.getItem('memo2.daily.'+dstr)||'[]');
        const isToday=dstr===todayStr;
        const hasTask=list.length>0;
        const doneAll=hasTask&&list.every(t=>t.done);

        const cell=doc.createElement('div');
        cell.style.cssText=`border:1px solid ${isToday?'#5C8DFF':'#e2e8f0'};border-radius:8px;padding:3px 4px;min-height:48px;display:flex;flex-direction:column;gap:2px;background:${isToday?'#eef3ff':'#fff'};`;

        const dayNum=doc.createElement('div');
        dayNum.textContent=String(d);
        dayNum.style.cssText=`font-size:10px;font-weight:${isToday?'700':'400'};color:${isToday?'#5C8DFF':'#374151'};margin-bottom:2px;`;
        cell.append(dayNum);
        ...
        list.slice(0,3).forEach(item=>{
          ...
          cb.type='checkbox';
          cb.checked=!!item.done;
          cb.style.cssText='width:10px;height:10px;cursor:pointer;flex-shrink:0;accent-color:#5C8DFF;';

          const txt=doc.createElement('span');
          txt.textContent=item.text;
          txt.style.cssText=`font-size:9px;color:${item.done?'#9aa5b1':'#374151'};${item.done?'text-decoration:line-through;':''}overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;`;
        ...
        if(list.length>3){
          ...
          more.style.cssText='font-size:9px;color:#9aa5b1;';
          cell.append(more);
        }
